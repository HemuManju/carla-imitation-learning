...
class BenchmarkRunner(object):
    ...
    def benchmark_agent(self, experiment_suite, agent, client):
        ...
        metrics_object = Metrics(
            experiment_suite.metrics_parameters, experiment_suite.dynamic_tasks
        )
        ...
        (
            ...
            start_experiment,
            start_rep,
        ) = self._recording.get_pose_experiment_rep(
            experiment_suite.get_number_of_poses_task(),
            experiment_suite.get_number_of_reps_poses(),
        )

        logging.info('START')

        for experiment in experiment_suite.get_experiments()[int(start_experiment) :]:
            positions = client.load_settings(experiment.conditions).player_start_spots
            ...
            for pose in experiment.poses[start_pose:]:
                for rep in range(start_rep, experiment.repetitions):
                    ...
                    logging.info('======== !!!! ==========')
                    logging.info(
                        ' Start Position %d End Position %d ', start_index, end_index
                    )

                    self._recording.log_poses(
                        start_index, end_index, experiment.Conditions.WeatherId
                    )

                    # Calculate the initial distance for this episode
                    initial_distance = sldist(
                        [
                            positions[start_index].location.x,
                            positions[start_index].location.y,
                        ],
                        [
                            positions[end_index].location.x,
                            positions[end_index].location.y,
                        ],
                    )
                    ...
                    (
                        ...
                        time_out,
                        positions[end_index],
                        str(experiment.Conditions.WeatherId)
                        + '_'
                        + str(experiment.task)
                        + '_'
                        + str(start_index)
                        + '.'
                        + str(end_index),
                        experiment_suite.metrics_parameters,
                    ...
                    if result > 0:
                        logging.info(
                            '+++++ Target achieved in %f seconds! +++++', final_time
                        )
                    else:
    ...
    def _has_agent_collided(self, measurement, metrics_parameters):
        ...
        if (
            measurement.collision_vehicles - self._previous_vehicle_collision
        ) > metrics_parameters['collision_vehicles']['threshold'] / 2.0:
            collided_veh = 1
        if (
            measurement.collision_pedestrians - self._previous_pedestrian_collision
        ) > metrics_parameters['collision_pedestrians']['threshold'] / 2.0:
            collided_ped = 1
        if (
            measurement.collision_other - self._previous_other_collision
        ...
        def search_closest_lane_point(x_agent, y_agent, depth):
            ...
            if not self._map.is_point_on_lane([x_agent, y_agent, 38]):
                ...
                result = search_closest_lane_point(
                    x_agent + step_size, y_agent, depth + 1
                )
    ...
    def _test_for_traffic_lights(self, measurement):
        ...
        def is_on_burning_point(_map, location):
            ...
            if not _map.is_point_on_intersection(
                [future_location_x, future_location_y, 38]
            ) and _map.is_point_on_intersection(
        ...
        for agent in measurement.non_player_agents:
            if agent.HasField('traffic_light'):
                if not self._map.is_point_on_intersection([player_x, player_y, 38]):
                    x_agent = agent.traffic_light.transform.location.x
                    y_agent = agent.traffic_light.transform.location.y
                    tl_vector, tl_dist = get_vec_dist(
                        x_agent, y_agent, player_x, player_y
                    )
                    if self._is_traffic_light_active(
                        agent, measurement.player_measurements.transform.orientation
                    ):
                        if (
                            is_on_burning_point(
                                self._map,
    ...
    def _run_navigation_episode(
        ...
        while not fail and not success:
            ...
            distance = sldist(
                [current_x, current_y], [target.location.x, target.location.y]
            )
        ...
        if success:
            return (
                ...
                measurement_vec,
                control_vec,
                float(current_timestamp - initial_timestamp) / 1000.0,
                distance,
                col_ped,
                col_veh,
                col_oth,
