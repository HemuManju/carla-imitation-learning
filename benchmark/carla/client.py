...
class CarlaClient(object):
    ...
    def start_episode(self, player_start_index):
        ...
        try:
            ...
            if not pb_message.ready:
                raise RuntimeError(
                    'cannot start episode: server failed to start episode'
                )
    ...
    def _request_new_episode(self, carla_settings):
        ...
        pb_message.ParseFromString(data)
        self._sensors = dict(
            (sensor.id, sensor) for sensor in _make_sensor_parsers(pb_message.sensors)
        )
...
def _make_sensor_parsers(sensors):
    ...
    getint32 = lambda data, index: struct.unpack('<L', data[index * 4 : index * 4 + 4])[
        0
    ]
    getint64 = lambda data, index: struct.unpack('<Q', data[index * 4 : index * 4 + 8])[
        0
    ]
    getfloat = lambda data, index: struct.unpack('<f', data[index * 4 : index * 4 + 4])[
        0
    ]
    ...
    def parse_lidar(data):
        ...
        point_count_by_channel = numpy.frombuffer(
            data[header_size : header_size + channels * 4], dtype=numpy.dtype('uint32')
        )
        points = numpy.frombuffer(
            data[header_size + channels * 4 :], dtype=numpy.dtype('f4')
        )
