...
class Metrics(object):
    ...
    def _divide_by_episodes(self, measurements_matrix, header):
        ...
        while i < measurements_matrix.shape[0]:
            ...
            if (
                current_start != prev_start and current_end != prev_end
    ...
    def _get_collisions(self, selected_matrix, header):
        ...
        while i < selected_matrix.shape[0]:
            if (
                selected_matrix[i, header.index('collision_other')]
                - selected_matrix[
                    (i - self._parameters['collision_other']['frames_skip']),
                    header.index('collision_other'),
                ]
        ...
        while i < selected_matrix.shape[0]:
            if (
                selected_matrix[i, header.index('collision_vehicles')]
                - selected_matrix[
                    (i - self._parameters['collision_vehicles']['frames_skip']),
                    header.index('collision_vehicles'),
                ]
        ...
        while i < selected_matrix.shape[0]:
            if (
                selected_matrix[i, header.index('collision_pedestrians')]
                - selected_matrix[
                    i - self._parameters['collision_pedestrians']['frames_skip'],
        ...
        return (
            count_collisions_general,
            count_collisions_vehicle,
            count_collisions_pedestrian,
        )
    ...
    def _get_out_of_road_lane(self, selected_matrix, header):
        ...
        while i < selected_matrix.shape[0]:
            ...
            if (
                selected_matrix[i, header.index('intersection_offroad')]
                - selected_matrix[
                    (i - self._parameters['intersection_offroad']['frames_skip']),
            ...
            if (
                ...
                - selected_matrix[
                    (i - self._parameters['intersection_otherlane']['frames_skip']),
    ...
    def _get_percentage_out_road(self, selected_matrix, header):
        ...
        while i < selected_matrix.shape[0]:
            ...
            if (
                (selected_matrix[i, header.index('intersection_offroad')])
                > self._parameters['intersection_offroad']['threshold']
                or selected_matrix[i, header.index('intersection_otherlane')]
                > self._parameters['intersection_otherlane']['threshold']
            ):
    ...
    def compute(self, path):
        ...
        result_matrix = np.loadtxt(
            os.path.join(path, 'summary.csv'), delimiter=",", skiprows=1
        )
        ...
        measurements_matrix = np.loadtxt(
            os.path.join(path, 'measurements.csv'), delimiter=",", skiprows=1
        )

        metrics_dictionary = {
            'episodes_completion': {w: [0] * len(tasks) for w in all_weathers},
            'intersection_offroad': {
                w: [[] for i in range(len(tasks))] for w in all_weathers
            },
            ...
            'percentage_green_lights': {w: [0] * len(tasks) for w in all_weathers},
            'end_pedestrian_collision': {w: [0] * len(tasks) for w in all_weathers},
            'end_vehicle_collision': {w: [0] * len(tasks) for w in all_weathers},
            'end_other_collision': {w: [0] * len(tasks) for w in all_weathers},
            'episodes_fully_completed': {w: [0] * len(tasks) for w in all_weathers},
        ...
        for t in range(len(tasks)):
            ...
            for w in weathers:
                ...
                experiment_results_matrix = result_matrix[
                    np.logical_and(
                        result_matrix[:, header.index('exp_id')] == tasks[t],
                        result_matrix[:, header.index('weather')] == w,
                    )
                ]

                experiment_metrics_matrix = measurements_matrix[
                    np.logical_and(
                        measurements_matrix[:, header_metrics.index('exp_id')]
                        == float(tasks[t]),
                        measurements_matrix[:, header_metrics.index('weather')]
                        == float(w),
                    )
                ]

                metrics_dictionary['end_pedestrian_collision'][w][
                    t
                ] = experiment_results_matrix[
                    :, header.index('end_pedestrian_collision')
                ].tolist()

                metrics_dictionary['end_vehicle_collision'][w][
                    t
                ] = experiment_results_matrix[
                    :, header.index('end_vehicle_collision')
                ].tolist()

                metrics_dictionary['end_other_collision'][w][
                    t
                ] = experiment_results_matrix[
                    :, header.index('end_other_collision')
                ].tolist()

                metrics_dictionary['percentage_green_lights'][w][t] = np.nan_to_num(
                    (
                        experiment_results_matrix[
                            :, header.index('number_green_lights')
                        ]
                        / (
                            experiment_results_matrix[
                                :, header.index('number_green_lights')
                            ...
                            + experiment_results_matrix[
                                :, header.index('number_red_lights')
                            ]
                        )
                    )
                ).tolist()

                metrics_dictionary['episodes_fully_completed'][w][
                    t
                ...
                for episode_experiment_metrics in episode_experiment_metrics_matrix:
                    ...
                    metrics_dictionary['average_speed'][w][t] = km_run_episodes / (
                        experiment_results_matrix[count, header.index('final_time')]
                        / 3600.0
                    )
                    count += 1
                    ...
                    metrics_dictionary['intersection_otherlane'][w][t].append(
                        lane_road[0]
                    )
                    metrics_dictionary['intersection_offroad'][w][t].append(
                        lane_road[1]
                    )

                    metrics_dictionary['percentage_off_road'][w][t].append(
                        self._get_percentage_out_road(
                            episode_experiment_metrics, header_metrics
                        )
                    )
                    ...
                    if tasks[t] in set(self._parameters['dynamic_tasks']):
                        ...
                        metrics_dictionary['collision_pedestrians'][w][t].append(
                            collisions[2]
                        )
                        metrics_dictionary['collision_vehicles'][w][t].append(
                            collisions[1]
                        )
                        metrics_dictionary['collision_other'][w][t].append(
                            collisions[0]
                        )

                    else:

                        metrics_dictionary['collision_pedestrians'][w][t].append(0)
                        metrics_dictionary['collision_vehicles'][w][t].append(0)
                        metrics_dictionary['collision_other'][w][t].append(0)
