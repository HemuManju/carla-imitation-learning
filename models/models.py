...
class SimpleLatentPredictionNet(nn.Module):
    ...
    def forward(self, x_in):
        ...
        if self.rnn_arch == "lstm":
            ...
            x_out_lat, (h_0, c_0) = self.rnn(
                x_in_lat[:, :-1, :], (h_0, c_0)
            )  # (batch, seq, hidden_size)
        elif self.rnn_arch == "gru":
            x_out_lat, h_0 = self.rnn(
                x_in_lat[:, :-1, :], h_0
            )  # (batch, seq, hidden_size)
...
class LightningSimpleLatentPredictionNet(pl.LightningModule):
    ...
    def loss(self, out, target, n_batch):
        ...
        loss_latent = (
            F.smooth_l1_loss(x_out_lat_temp, x_in_lat_temp, reduction='sum') / n
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, loss_num = self.loss(
            (x_out, x_out_ae, x_out_lat, x_in_lat), batch[0], n_batch=batch[0].shape[0]
        )
...
class SimpleLatentSensorPredictionNet(SimpleLatentPredictionNet):
    ...
    def __init__(self, config, autoencoder: torch.nn.Module, device):
        ...
        self.fc_sensor_in = nn.Linear(
            in_features=self.sensor_size, out_features=self.sensor_size
        )
        self.fc_sensor_out = nn.Linear(
            in_features=self.sensor_size, out_features=self.sensor_size
        )
    ...
    def forward(self, x_in, s_in):
        ...
        s_in = s_in.reshape(-1, ss[2])  # (batch*(seq+1), s)
        s_in = self.fc_sensor_in(s_in)
        s_in = s_in.reshape(ss[0], ss[1], ss[2])

        # Encode all image sequence (n_batch x (n_seq+1)) -> latent representation
        x_in = x_in.reshape(-1, xs[2], xs[3], xs[4])  # (batch*(seq+1),  c, h, w)
        x_in_lat = self.encode(x_in)  # (batch*(seq+1),  input_size)
        x_out_ae = self.decode(x_in_lat)  # Decode again
        x_in_lat = x_in_lat.reshape(xs[0], xs[1], -1)  # (batch, seq, input_size)

        # Concatenate latent representation with sensor measurements
        concat_in = torch.cat(
            (x_in_lat, s_in), dim=2
        )  # (batch, seq, latent_size+sensor_size)

        if self.h_0_latent:
            h_0 = torch.unsqueeze(concat_in[:, 0, :], 0).contiguous()
        else:
            h_0 = torch.zeros(
                self.num_layers, xs[0], self.latent_size + self.sensor_size
            ).to(self.device)
