...
class Model_Segmentation_Traffic_Light_Supervised(nn.Module):
    def __init__(
        ...
        self.example_input_array = [torch.randn((32, 3, 168, 288))]
        ...
        # self.up_sampled_block_4 = create_resnet_basic_block(168, 288, 64, 32)
        ...
        ###semseg calsses: moving obstacles, traffic lights, road markers, road, sidewalk, background
    ...
    def forward(self, x):
        ...
        dist_state = self.fc1_dist_to_frontcar(classif_state_net)
        dist_state = nn.functional.relu(dist_state)
        dist_to_frontcar = self.fc2_dist_to_frontcar(dist_state)


        return out_seg, tl_state_output, dist_to_tl_output, dist_to_frontcar
